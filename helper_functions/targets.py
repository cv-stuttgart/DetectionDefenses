...
def scaled_constant_flow(flow,scaling):
    """Produces tensor with constant value `scaling` with the same size as flow

    Args:
        flow (tensor): input flow field
        scaling (float): value of the flow

    Returns:
        tensor: constant flow field with same dimension as the input
    """
    return torch.full_like(flow, float(scaling))
...
def scaled_random_flow(flow, scaling=1.):
    """Produces a random noise filed with a specified scaling that is applied to the flow vectors
    that have a standard length of <= sqrt(2)
    ...
    Returns:
        tensor: random, scaled flow field
    """
    # torch.manual_seed(3)
    target_lengths = float(scaling) * torch.rand(flow.size())
    target_signs = torch.sign(torch.rand(flow.size()) - 0.5)
    return target_lengths * target_signs
...
def get_target(target_name, flow_pred_init, flow_target_scale=1., custom_target_path="", device=None):
    ...
    elif target_name == 'scaled_constant_flow':
        target = scaled_constant_flow(flow_pred_init, scaling=flow_target_scale)
    elif target_name == 'scaled_random_flow':
        target = scaled_random_flow(flow_pred_init, scaling=flow_target_scale)
    else:
        raise ValueError('The specified target type "' + target_name + '" is not defined and cannot be used. Select one of "zero", "neg_flow", "custom", "scaled_random_flow" or "scaled_constant_flow". Aborting.')
